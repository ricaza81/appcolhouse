                  ...
                  <div class="col-md-3">
                        <a  target="_self">
                            ...
                            <div class="text-center" style="margin-top:-120px">
                             ...
                             <div class="text-right" style="margin-top:10px;padding:15px">
                               <a href="{{ route('admin.properties_sub.show',[$tenant->subproperty->id]) }}" target="_blank">
                                <h4 style="font-weight:400;color:#fff;line-height:10px;">{{ $tenant->subproperty->nombre }} [Ver]
                                </h4>
                              </a>
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <td field-key='porpagar'>
                             ...
                             <span class="label label-info" style="padding-bottom:0px;padding-top:8px;font-weight:200;">
                                <a href=""data-toggle="tooltip" title="Ver Facturas" style="color:#fff">
                                 <h4 style="font-weight:200;font-size:15px">Valor total facturas:<br/>${{number_format(($tenant->facturas2($tenant->id))), 0, ',', '.'}}
                                </h4>
                                 <h4 style="font-weight:200;font-size:15px">Valor total pagado:<br/>${{number_format(($tenant->facturas_pagadas($tenant->id))), 0, ',', '.'}}
                                </h4>
                             </a>
                         </span>
                            </td>
                        ...
                        <br/>
                            ...
                            <h6>Cliente creado el:<br/>
                                {{date('d/F/Y', strtotime($tenant->created_at))}}</h6>
                            </td>
                    </table>
                    ...
                    <table class="table table-bordered table-striped">
                        <tr>
                            <th>Codeudor</th>
                            <td field-key='name' style="text-transform:capitalize;">{{ $tenant->codeudor }}</td>
                        </tr>
                        <tr>
                            <th>Cédula</th>
...
<table id="lista_facturas" class="table table-bordered table-striped {{ count($facturas) > 0 ? 'datatable' : '' }} @can('properties_facturas_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        ...
                        <th>@lang('Pendiente por pagar')</th>
                        <th>Estado</th>
                        @if( request('show_deleted') == 1 )
                        <th>&nbsp;</th>
                        @else
...
@endif
                                ...
                                <td data-sort="{{strtotime($factura->fecha_inicio)}}" field-key='fecha_inicio'>{{date('d/F/Y', strtotime($factura->fecha_inicio)) }}
                                ...
                                <td field-key='name'style="font-size:15px;font-weight: 700">${{ number_format($factura->valor_neto), 0, ',', '.' }}</td>
                                 <td field-key='name'style="font-size:15px;font-weight: 700">
                                    $
                                    {{ number_format($factura->valor_neto-$factura->pago2($factura->id)), 0, ',', '.' }}
                                    ...
                                    <span>
                                           Fecha pago: {{date('d/F/Y',strtotime($factura->pago_valor->fecha_pago))}}
                                       </span>
