...
class PropertiesFacturas extends Model
{
    ...
    public function tenant()
    {
       ...
       public function tenant_name($id)
    {
     
       $factura=PropertiesFacturas::findOrFail($id);
       $id_inquilino=$factura->id_tenant;
       $tenant=DB::table('users')->where('id','=',$id_inquilino)->where('deleted_at',NULL)->pluck('name')->first();
       return ($tenant);
    }

         public function tenant_id($id)
    {
     
       $factura=PropertiesFacturas::findOrFail($id);
       $id_inquilino=$factura->id_tenant;
       $tenant=DB::table('users')->where('id','=',$id_inquilino)->where('deleted_at',NULL)->pluck('id')->first();
       return ($tenant);
    }
    ...
    {
        $factura=PropertiesFacturas::findOrFail($id);
        $pago=DB::table('properties_pagos')->where('id_factura',$factura->id)->where('deleted_at',NULL)->pluck('id','valor')->first();
        //$id_pago=$pago->id;
        return($pago);
    }
