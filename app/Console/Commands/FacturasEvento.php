...
class FacturasEvento extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($unidades as $unidad)    {
             if ($unidad->cuenta_inquilinos2($unidad->id) > 0) {
                ...
                $pdf = \PDF::loadView('admin.properties_facturas.facturapdf',compact('factura','facturas','fecha_fin_contrato','fecha_limite_renovacion'));
                 $asunto='Generación de facturas automaticas (informe automático)';
        $mes=date('m');
        ...
        $total_facturas=count($facturas);
         ...
         Mail::send('correo.plantilla_correo', $data, function ($message) use ($asunto,$facturas,$total_facturas,$pdf) {
